...
func TestComputeRegen(t *testing.T) {
	...
	tests := []struct {
		...
		{"cold disarms", true, "cold", rpm, 50000, vMax, iMax, false, "cold"},
		{"hot disarms", true, "hot", rpm, 50000, vMax, iMax, false, "hot"},
		{"disabled", false, "none", rpm, 50000, vMax, iMax, false, "off"},
		{"standstill", true, "none", 0, 50000, vMax, iMax, false, "standstill"},
		{"just below engage speed", true, "none", regenEngageMinWheelRPM - 1, 50000, vMax, iMax, false, "standstill"},
		{"at engage speed", true, "none", regenEngageMinWheelRPM, 50000, vMax, iMax, true, "none"},
		{"no caps -> assume available", true, "none", rpm, 50000, 0, 0, true, "none"},
		{"current-limited below cap", true, "none", rpm, 50000, vMax, iMax, true, "none"},
		{"voltage-limited to zero at cap+band", true, "none", rpm, vMax + regenVLoopBandMV, vMax, iMax, false, "full"},
		// Authority 3900 counts (iMax 40000 mA); 1 V over-cap costs 1875 counts,
		// leaving a positive envelope -> still available.
		{"voltage backing off inside band", true, "none", rpm, vMax + 1000, vMax, 40000, true, "none"},
	}
...
func TestComputeRegenStandstillBeatsVoltageHeadroom(t *testing.T) {
	got := computeRegen(true, "none", 0, 40000, 56000, 10000)
